...
const EX1_NODES: Node<WorkflowNodeData>[] = [
  ...
  },
  {
    ...
    data: {
      kind: "audience", title: "Audience", subtitle: "CSV · primary key customer_id", valid: true, preset: true, locked: true,
      config: {
        audienceMode: "csv",
...
const EX2_NODES: Node<WorkflowNodeData>[] = [
  ...
  },
  {
    ...
    data: {
      kind: "audience", title: "Audience", subtitle: "Runtime API · JSON payload", valid: true, preset: true, locked: true,
      config: {
        audienceMode: "api",
        payloadType: "list",
        fields: EX2_FIELDS,
...
const sAud = (subtitle: string, keys: string[]): Spec => ({
  // Audience is the single non-deletable contact entry point — every canvas ships
  // with exactly one, and it's dropped from the addable palette so users can't add
  // a second one either. `locked: true` prevents deletion via the UI.
  id: "aud", kind: "audience", title: "Audience", subtitle, locked: true,
  config: {
