...
/* ---- Voice Agent ↔ tools (for the campaign builder's Voice Agent node) ---- */

export type VoiceAgentDef = { id: string; tools: string[] };

/** Keyed by the persona string the voice node stores in `config.agent`. */
export const VOICE_AGENTS: VoiceAgentDef[] = [
  { id: "Aria · Conversational", tools: ["place_call", "order_lookup", "crm_query"] },
  { id: "Kai · Formal", tools: ["place_call", "crm_query", "refund_initiate"] },
  { id: "Maya · Friendly", tools: ["place_call", "customer_context", "order_lookup"] },
];

export const AGENT_TOOLS: Record<string, string[]> = Object.fromEntries(
  VOICE_AGENTS.map((a) => [a.id, a.tools]),
);

export function agentToolDefs(agentId?: string): ToolDef[] {
  const handles = agentId ? AGENT_TOOLS[agentId] ?? [] : [];
  return handles.map((h) => getTool(h)).filter((t): t is ToolDef => !!t);
}

export type ToolCampaignSlot = { tool: string; key: string; column: string; description: string };

/** Campaign-sourced inputs across an agent's tools — each needs a CSV column at the node. */
export function campaignSlotsForAgent(agentId?: string): ToolCampaignSlot[] {
  const slots: ToolCampaignSlot[] = [];
  for (const t of agentToolDefs(agentId)) {
    for (const inp of t.inputs) {
      if (inp.source === "campaign") {
        slots.push({ tool: t.handle, key: inp.key, column: inp.value ?? inp.key, description: inp.description });
      }
    }
  }
  return slots;
...
export function toolOutputVarsForAgent(agentId?: string): { key: string; source: string }[] {
  const out: { key: string; source: string }[] = [];
  for (const t of agentToolDefs(agentId)) {
    for (const o of t.outputs) out.push({ key: `${t.handle}.${o.varName}`, source: `@${t.handle}` });
  }
  return out;
}
