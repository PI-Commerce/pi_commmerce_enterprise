...
const ROUTES: { match: (p: string) => boolean; ctx: PiContext }[] = [
  ...
  },
  {
    ...
    ctx: {
      scope: "Analytics",
      placeholder: "Ask Pi to chart call outcomes, intent, or sentiment…",
      chips: ["Why do most calls end with no meaningful engagement?", "Where are we losing interested leads?", "Break down customer sentiment across calls"],
      thinking: ["Reading the voice-agent call log…", "Scoring intent & sentiment across 459 calls…", "Composing a chart…"],
      result: {
        text: "Across the last 459 voice calls, 64% end with no meaningful engagement and only ~7% reach buying intent. I can pin this as a card or export it as a PNG.",
        diff: ["+ chart  Call outcomes by intent (grouped bar)", "+ source  Volt Money voice agent · last 459 calls"],
        cta: "Add as card",
      },
      nudge: {
        id: "analytics_no_engagement",
        label: "64% of calls end with no meaningful engagement — see why?",
        prompt: "Why do most calls end with no meaningful engagement?",
      },
    },
  },
  {
