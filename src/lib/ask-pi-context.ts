...
const ROUTES: { match: (p: string) => boolean; ctx: PiContext }[] = [
  ...
  },
  {
    ...
    ctx: {
      ...
      chips: ["Why did reactivation drop 8%?", "Chart conversions by channel", "Compare this run vs last"],
      thinking: ["Reading the current dashboard scope…", "Pulling channel-level conversions…", "Composing a new chart…"],
      result: {
        text: "Here's conversion split by channel for the selected campaign — Voice leads WhatsApp by 11 points on win-back. I can pin this as a card or export it as a PNG.",
        diff: ["+ chart  Conversions by channel (grouped bar)", "+ source  current campaign · latest run"],
        cta: "Add as card",
      },
      nudge: {
        id: "analytics_reactivation_drop",
        label: "Reactivation conversion dropped 8% after Jun 7 — see why?",
        prompt: "Why did reactivation drop 8%?",
      },
    },
  },
  {
