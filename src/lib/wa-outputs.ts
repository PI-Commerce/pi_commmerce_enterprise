 ...
 *
 * SMS also reuses this exported function today (see `actionNodeOutputs`); the
 * two handles are the right shape for SMS too (delivered/failed) once the SMS
 * flow adopts them. Legacy handle id `completed` is preserved on this shape
 * (kept out of the exported list) so existing runs / analytics that reference
 * it don't break; new campaigns always wire from `success` or `failure`.
 */
