...
import {
  ...
  FileSpreadsheet, Loader2, Clock, Hash, Pencil,
} from "lucide-react";
...
function AiChatCore({ config, readOnly, mark, onChange }: { config?: PresetConfig; readOnly?: boolean; mark: (v: boolean, e?: string) => void; onChange: (patch: Partial<WorkflowNodeData>) => void }) {
  ...
  // Two fixed handles (Completed / Fallback), mirroring the Voice node; persist
  // config so the node restores on reopen.
  useEffect(() => {
    onChange({
      outputs: chatOutputs(),
...
function ActionAdvanceBanner({ kind, type1, timeoutHours }: { kind: ActionKind; type1?: boolean; timeoutHours?: number }) {
  ...
  const text =
    kind === "voiceCall"
      ...
      : kind === "aiChat"
        ? "Branch with a Conditional downstream for the specific outcome."
      : kind === "sms"
