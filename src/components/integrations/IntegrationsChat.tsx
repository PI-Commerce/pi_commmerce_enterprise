/**
 * Ask Pi Integrations chat — multi-turn Q&A over the vendor docs on /integrations.
 *
 * Why a dedicated shell (mirroring AnalyticsChat) instead of the single-shot
 * PiResultCard the rest of the dock uses:
 *
 ...
 * - RAG answers are markdown-heavy (bold section names, code-fenced scopes,
 *   deep links). A chat transcript renders that natively via
 *   renderChatMarkdown; the result-card path treats every reply as one
 *   opaque paragraph.
 * - Persona / harness: Pi introduces itself with an intro turn on mount
 *   that spells out what it can and can't help with on this surface, so
 *   the user isn't left guessing at scope.
 *
 * Session-only history (no D1 persistence — matches AnalyticsChat).
 */
...
const INTRO_TURN: Turn = {
  id: "intro",
  role: "assistant",
  text: "Hi, I'm Pi. On this page I can walk you through **connecting a vendor** — right now that's **Paytm Payment Gateway**, **CleverTap**, and **Shopify**. Ask me anything about prerequisites, credentials, setup, testing, or troubleshooting for those three and I'll cite the doc section I'm reading from.\n\nWhat I **can't** help with here: analytics questions, campaign edits, agent authoring, account settings. Head to the matching surface for those and I'll be more useful.",
};

const STARTER_CHIPS: string[] = [
  "How do I connect Shopify?",
  "Where do I find my Paytm API keys?",
  "What data does CleverTap sync?",
];

export function IntegrationsChat({ onClose }: { onClose: () => void }) {
  ...
  // Match AnalyticsChat's drag-to-resize header — same interaction pattern
  // so users don't have to relearn it across surfaces.
  ...
  // transcript grows. Behaviour is smooth, not instant, so the transition
  // reads as a chat, not a jump.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col overflow-hidden">
      {/* Header — draggable resize handle + close. Mirrors AnalyticsChat so
          the two chat surfaces feel like siblings. */}
      <div
      ...
      {/* Scope ribbon — non-technical, always visible. Same shape as the
          analytics "Answering about" ribbon so the two chats read as a
          consistent pattern. */}
      <div className="border-b border-border/60 bg-muted/30 px-4 py-1.5">
        <div className="flex items-baseline gap-1.5 text-[11.5px] leading-snug">
          ...
          <span className="min-w-0 truncate font-medium text-foreground">
            connecting Paytm Payment Gateway · CleverTap · Shopify
          </span>
        </div>
      </div>
